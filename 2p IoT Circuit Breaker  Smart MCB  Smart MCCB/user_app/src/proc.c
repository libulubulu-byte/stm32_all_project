...
{
						uint8_t za_flag=0,tflag=0,ts_flag=0,FlagSta=0;
...
//												baifen_he=0.30;
													...
													adcvaluep2= ADC_Getvalue(9);//heza
													if(adcvaluep2_avr*0.80<adcvaluep2-adcvaluep2_m)
													{
														adcvaluep2= ADC_Getvalue(9);//heza
														if(adcvaluep2_avr*0.80<adcvaluep2-adcvaluep2_m)
														{
...
static void attreadiup(void) { 
  static uint32_t att7053currenttick = 0;
  uint8_t temp[4];
  read_rms(ATT7053_REG_I1_RMS, &workdata.attdata.leakageamp,  0.000662148);
  read_rms(ATT7053_REG_I2_RMS, &workdata.attdata.amp,  0.023144826);
  if (TMS_IS_OVER(att7053currenttick, 10)) {
    TMS_INIT(att7053currenttick);
    if(att7053currenttick>(att7053currenttick+10)){
      att7053currenttick=0;
    }
    read_att7053bu(ATT7053_REG_PF_CNT, temp);
    read_rms(ATT7053_REG_V_RMS, &workdata.attdata.volt, 0.010445654);
		
    read_freq(&workdata.attdata.freq);
    read_power(ATT7053_REG_POW_P2, &workdata.attdata.power_p, 0.020542932);
    read_power(ATT7053_REG_POW_Q2, &workdata.attdata.power_q, 0.020542932);
    read_power(ATT7053_REG_POW_S, &workdata.attdata.power_s, 0.020542932);
    workdata.attdata.power_ps=workdata.attdata.amp*workdata.attdata.volt*workdata.attdata.power_p/workdata.attdata.power_s/100;
...
static void attreadenergy(void) {
  static uint32_t att7053energytick = 0;
  uint16_t curenergy = 0;

  read_energy(ATT7053_REG_ENE_Q, &curenergy, workpara.coefficient.ec);
  workpara.pulse_mantissa_q += curenergy;
  energycumulative(&workpara.pulse_mantissa_q, &workpara.total_enery_q);

  read_energy(ATT7053_REG_ENE_S, &curenergy, workpara.coefficient.ec);
  workpara.pulse_mantissa_s += curenergy;
  energycumulative(&workpara.pulse_mantissa_s, &workpara.total_enery_s);
    
  read_energy(ATT7053_REG_ENE_P, &curenergy, workpara.coefficient.ec);
  workpara.pulse_mantissa_p += curenergy;
  energycumulative(&workpara.pulse_mantissa_p, &workpara.total_enery_p);

  if (TMS_IS_OVER(att7053energytick, 3600000)) { 
    TMS_INIT(att7053energytick);
    saveparam();
  }
}

void att7053pro(void) {
  attreadiup();
  attreadenergy(); 
}
...
//      Flag_RepeatSwitch = 1;  //1019
  ...
  Flag_leakagetest=0;
    if ((workdata.attdata.volt > workpara.limit_volt_high)) 
			{
				HAL_Delay(10);
				read_rms(ATT7053_REG_V_RMS, &workdata.attdata.volt, 0.010445654);
    ...
    } else if ((workdata.attdata.volt < workpara.limit_volt_low)&&(workdata.attdata.volt >100) ) 
		{
				dianya=workdata.attdata.volt;
				HAL_Delay(10);
				read_rms(ATT7053_REG_V_RMS, &workdata.attdata.volt, 0.010445654);
				if ((workdata.attdata.volt < workpara.limit_volt_low)&&(workdata.attdata.volt >100) ) 
					{
...
//		workdata.attdata.amp=14500;
			if ((workdata.attdata.amp >= (workpara.limit_amp*90/100)) && (workdata.attdata.amp < (workpara.limit_amp*145/100))) { //过流事件 
				Flag_cross_limitamp=1;
      }else
			{
				Flag_cross_limitamp=0;
			}
    if ((workdata.attdata.amp >= (workpara.limit_amp*113/100)) && (workdata.attdata.amp < (workpara.limit_amp*145/100))) { //过流事件
...
//        TMS_INIT(amp_tick3);
//      }
    }else if(((workdata.attdata.amp) >= (workpara.limit_amp*145/100)) && (workdata.attdata.amp < (workpara.limit_amp*255/100))){
    ...
    }else if((workdata.attdata.amp >= (workpara.limit_amp*255/100)) && (workdata.attdata.amp < (workpara.limit_amp*5))){
    ...
    }else if(workdata.attdata.amp >= (workpara.limit_amp*5)){
      Brakefailure=0;
      Flag_currmax=1; //1019
      Flag_RepeatSwitch = 1;
...
static  uint8_t currentinfopack(uint8_t* pdata, uint8_t* plen) {
  uint8_t* p = NULL;
	double buchang_baifen=0.97654321;
	uint32_t total_enery_p=0;
	total_enery_p=workpara.total_enery_p;
	total_enery_p=(uint32_t)(total_enery_p*buchang_baifen);
  p = pdata;
  *p++ = workpara.type;
  *p++ = workpara.np;
  *p++ = (uint8_t)(workpara.mode >> 8);
  ...
  *p++ = (uint8_t)(workdata.attdata.power_ps >> 8);
  *p++ = (uint8_t)(workdata.attdata.power_ps);
  *p++ = (uint8_t)(workdata.attdata.volt >> 24);
  *p++ = (uint8_t)(workdata.attdata.volt >> 16);
  *p++ = (uint8_t)(workdata.attdata.volt >> 8);
  *p++ = (uint8_t)(workdata.attdata.volt);
  *p++ = (uint8_t)(workdata.attdata.amp >> 24);
  *p++ = (uint8_t)(workdata.attdata.amp >> 16);
  *p++ = (uint8_t)(workdata.attdata.amp >> 8);
  *p++ = (uint8_t)(workdata.attdata.amp);
  *p++ = (uint8_t)(workdata.attdata.power_p >> 8);
  *p++ = (uint8_t)(workdata.attdata.power_p);
...
int8_t cmddataproc(uint8_t* pdata, uint8_t len) 
{
	...
	switch (order) {
		...
		case CMD_CHECK_ONLINE:
			onlineset(1);//设置为在线
			askdata[0] = workpara.type;//返回设备类型
			asklen = 1;
			break;

		case CMD_INQUIRE_BASIC_INFO:
			basicinfopack(askdata, &asklen);//基本信息打包
			break;

		case CMD_INQUIRE_CURRENT_INFO:
			currentinfopack(askdata, &asklen);//当前信息打包
			break;

		case CMD_READ_WORK_PARA:
			readpara(askdata, para_num, &asklen);//读取属性
			break;

		case CMD_WRITE_WORK_PARA:
			writepara(askdata,pdata,&asklen);//设置参数
			break;
		
		case CMD_SET_TEMPE:
			memcpy(&status, pdata + 9, 2);
			int16tBS((uint16_t *)&status);
			workpara.limit_temperature = status;//设置温度阈值
			saveparam();
			break;
		...
		case CMD_SET_LIMITVOLAT_H: 
		{
			...
			int32tBS(&status);
			workpara.limit_volt_high = status;//设置电压上限
			saveparam();
			break;
		}
		...
		case CMD_SET_LIMITVOLAT_L: 
		{
			...
			memcpy(&status, pdata + 9, 4);
			int32tBS(&status);
			workpara.limit_volt_low = status;//设置电压下限
			saveparam();
			break;
		}
		...
		case CMD_SET_SWITCH: 
		{
			...
			if (hand_switch_state==1) {
				if(status==0){
					workdata.switchhopestatus=0;//开关期望状态，远程控制
				}else{
					workdata.switchhopestatus=1;
				}         
				askdata[0] = 0;
		...
		case CMD_SET_LIMITAMP_PARA: 
		{
			...
			memcpy(&status, pdata + 9, 4);
			int32tBS(&status);
			workpara.limit_amp = status;//电流阈值
			askdata[0] = 0;
			asklen = 1;
			saveparam();
			break;
		...
		case CMD_SET_LIMITPOWER_PARA: 
		{
			...
			memcpy(&status, pdata + 9, 4);
			int32tBS(&status);
			workpara.limit_power_s = status;//功率阈值
			askdata[0] = 0;
			asklen = 1;
			saveparam();
			break;	
		...
		case CMD_READ_ALL_WORK_PARA: 
		{
			...
			memcpy(askdata, (uint8_t*)&workpara, asklen);//读取全部工作参数
			break;
		}
...
void adjust_cali_para_all(void) {
  ...
  adjust_cali_para(ATT7053_REG_HFCONST, workpara.calibration.hf_const);
  read_att7053bu(ATT7053_REG_HFCONST, debug_temp);
  adjust_cali_para(ATT7053_REG_GP1, workpara.calibration.gp1);
  adjust_cali_para(ATT7053_REG_GQ1, workpara.calibration.gp1);
  adjust_cali_para(ATT7053_REG_GS1, workpara.calibration.gp1);
  adjust_cali_para(ATT7053_REG_GPHASE1, workpara.calibration.gphase1);
  adjust_cali_para(ATT7053_REG_P1_OFT_H, (uint8_t)(workpara.calibration.poffset1 >> 8));
  adjust_cali_para(ATT7053_REG_P1_OFT_L, (uint8_t)(workpara.calibration.poffset1));
  
  write_att7053bu(ATT7053_REG_WP, 0x00);
}
