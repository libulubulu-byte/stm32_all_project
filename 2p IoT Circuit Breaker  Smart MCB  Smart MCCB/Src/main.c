...
double baifen_he=0.40;
double baifen_fen=0.40;
double baifen2_he=0.18;
double baifen2_fen=0.126;
uint32_t timeout=1;
/* USER CODE BEGIN PFP */
...
void zidonghuoqu()
{
	uint16_t i=0,k=0,la_flag=0;
			...
			SW_break();
			while(adcvaluep4>adcvaluep3)
			{
				huoqumax_min();

			}
			while(adcvaluep3>adcvaluep4)
			{
				huoqumax_min();
			}
			...
			while(la_flag==0)
			{
						...
						if(adcvaluep2_avr*baifen2_he<adcvaluep2-adcvaluep2_m&&SW_ON)
						{
							if(SW_ON)
							{
								...
								HAL_TIM_PWM_Stop(&htim15, TIM_CHANNEL_2);la_flag=1;
								last_switchstate=1;
						}
				}	
...
int main(void)
{
	...
	uint16_t data32=0;uint32_t Address=0;
...
//		zidonghuoqu();
//		dengdai_queren();
//		if(!(baifen2_he==1||baifen2_fen<0.03))
//		{
//			flash_beifen();
//		}
//	}
...
//  }
   while (1){
    ...
    if (TMS_IS_OVER(att7053currenttick, 100)) {
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : SET_NUMBER_IN_Pin OV_DETE_Pin UV_DETE_Pin ATT7053_IRQ_Pin */
  GPIO_InitStruct.Pin = OV_DETE_Pin|UV_DETE_Pin|ATT7053_IRQ_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  ...
  /*Configure GPIO pins : DEBUG_RX_Pin ATT7053_PF_Pin ATT7053_QF_Pin */
  GPIO_InitStruct.Pin = DEBUG_RX_Pin|ATT7053_PF_Pin|ATT7053_QF_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
