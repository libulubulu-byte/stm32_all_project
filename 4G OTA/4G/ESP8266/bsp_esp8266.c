...
bool ESP8266_Cmd3 ( char * cmd, char * reply1, char * reply2 )
{ 
	uint32_t i=0,delay_count=0;
	strEsp8266_Fram_Record .InfBit .FramLength = 0;               //从新开始接收新的数据包
	macPC_Usart ( "AT——send:%s\r\n", cmd );
	macESP8266_Usart ( "%s\r\n", cmd );
	...
	while(1)
	{
		Delay_ms(100);
		delay_count++;
		if(strstr(strEsp8266_Fram_Record .Data_RX_BUF,reply1))
		{
 ...
 * 调用  ：被外部调用
 */
bool ESP8266_Cmd2 ( char * cmd, char * reply1, char * reply2, u32 waittime )
{    
	strEsp8266_Fram_Record .InfBit .FramLength = 0;               //从新开始接收新的数据包
...
bool ESP8266_Net_Mode_Choose ( ENUM_Net_ModeTypeDef enumMode )
{
	...
	{
		case STA:
			return ESP8266_Cmd ( "AT+CWMODE=1", "OK", "no change", 4500 ); 
		
	  case AP:
		  return ESP8266_Cmd ( "AT+CWMODE=2", "OK", "no change", 4500 ); 
		
		case STA_AP:
		  return ESP8266_Cmd ( "AT+CWMODE=3", "OK", "no change", 4500 ); 
...
bool ESP8266_Enable_MultipleId ( FunctionalState enumEnUnvarnishTx )
{
	...
	return ESP8266_Cmd ( cStr, "OK", 0, 2500 );
